  ...
  self.db.transaction(function (tx) {
    ...
    tx.executeSql(deleteSql, [self.name, id], function () {
      ...
      Object.keys(keyValues).forEach(function (key) {
        var value = JSON.stringify(keyValues[key]);
        tx.executeSql(insertSql, [self.name, id, key, value]);
      });
    });
  }, unknownError(callback), function () {
    callback();
...
Index.prototype.get = utils.toPromise(function (start, end, callback) {
  var self = this;
  ...
  if (typeof start !== 'undefined' && typeof start !== 'string') {
    return callback(errors.error(errors.WSQ_ERROR, 'start must be a string if defined',
      'start-must-be-string'));
  }
  if (typeof end !== 'undefined' && typeof end !== 'string') {
    return callback(errors.error(errors.WSQ_ERROR, 'end must be a string if defined',
      'end-must-be-string'));
  }
  ...
  self.db.transaction(function (tx) {
    ...
    var sql = 'SELECT id, key, value FROM ' + PLUGIN_DATA_STORE + ' WHERE ' +
      (criteria.join(' AND ')) + ' ORDER BY key';
    tx.executeSql(sql, sqlArgs, function (tx, result) {
      for (var i = 0; i < result.rows.length; i++) {
        var item = result.rows.item(i);
        results.push({
