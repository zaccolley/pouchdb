...
function LevelPouch(opts, callback) {
  ...
  api.index = function (name) {
    ...
    return new Index(db.sublevel('index-' + name));
  };
}
...
Index.prototype.destroy = utils.toPromise(function (callback) {
  this.db.createKeyStream().pipe(deleteStream(this.db, callback));
});
...
Index.prototype.put = utils.toPromise(function (id, map, callback) {
  ...
  self.byDoc.get(id, function (err, resp) {
    ...
    var payload = Object.keys(map).forEach(function (key) {
      batch.put(key, map[key], {
        prefix: self.byKey
      });
      return key;
    });
...
Index.prototype.get = utils.toPromise(function (from, to, callback) {
  if (typeof to === 'function') {
    return this.byKey.get(from, callback);
  }
  var output = [];
  var stream = this.byKey.createReadStream({
    start: from,
    end: to
  });
