...
function createDocMap(db) {
  return mapReduce(db, 'docCount', function (key, obj, emit) {
      if (obj.deleted) {
        emit(['all', key], '0');
      } else {
        emit(['all', key], '1');
...
function LevelPouch(opts, callback) {
  ...
  function afterDBCreated() {
    ...
    mapDB = createDocMap(stores.docStore);
  ...
  api.docCount = function (cb) {
    return mapDB.get(['all'], function (err, resp) {
      if (err) {
        cb(err);
      } else {
        cb(null, parseInt(resp, 10));
      }
    });
  };
