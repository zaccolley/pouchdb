...
AbstractPouchDB.prototype.changes = function (opts, callback, promise) {
  var cancelled = false;
  // promise should be set iff this call was deferred through taskqueue
  if (promise) {
    doChanges(this, opts, callback, promise);
    return promise;
  }

  function doChanges(api, opts, callback, promise) {
    opts = utils.extend(true, {}, opts);
    ...
    if (opts.since === 'latest') {
      api.info(function (err, info) {
        ...
        doChanges(api, opts, callback, promise);
      });
      return;
    }
    ...
    var new_promise = api._changes(opts);
    var cancel = promise.cancel;
    promise.cancel = function (reason) {
      // TODO: cancel new_promise first, after _changes is fixed to
      // return an error on cancel
      cancel.apply(this, reason);
      new_promise.cancel(reason);
    };
  }

  // Custom version of toPromise to pass promise
  function toPromise(func) {
    return function () {
      ...
      var promise = new Promise(function (fulfill, reject) {
        ...
        // Defer calling func until promise is set
        process.nextTick(function () {
          args.push(callback);
          args.push(promise);
  ...
  var changes = toPromise(function (opts, callback, promise) {
    if (typeof opts === 'function') {
      promise = callback;
      callback = opts;
      opts = {};
    ...
    var orig_cancel = promise.cancel;
    promise.cancel = function () {
      callback(null, {status: 'cancelled'});
      orig_cancel.apply(this, arguments);
    };

    if (!this.taskqueue.isReady) {
      this.taskqueue.addTask('changes', [opts, callback, promise]);
      return;
    }
