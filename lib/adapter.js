...
utils.inherits(AbstractPouchDB, Writable);
...
function AbstractPouchDB() {
  ...
  self.end = function () {
    //no opp, we don't want to close this
  };
...
AbstractPouchDB.prototype._write = function (docs, _, next) {
  if (!Array.isArray(docs)) {
    docs = [docs];
  }
  this.bulkDocs({docs: docs}, {new_edits: false}, function (err, res) {
    next(err, res);
  });
};